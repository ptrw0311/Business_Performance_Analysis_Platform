    ...
    if (idx === 0) {
      ...
      html = (
        <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '10px', alignItems: 'center', marginBottom: '8px' }}>
          ...
          <div>
            <span className={rClass}>{rIcon} {revDiff >= 0 ? '成長' : '下滑'} {Math.abs(revDiff).toLocaleString()}</span>
      ...
      html += (
        <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '10px', alignItems: 'center', marginBottom: '8px' }}>
          ...
          <div>
            <span className={pClass}>{pIcon} 淨利{proDiff >= 0 ? '增加' : '減少'} {Math.abs(proDiff).toLocaleString()}</span>
