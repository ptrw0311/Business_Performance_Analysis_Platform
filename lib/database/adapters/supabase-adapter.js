...
import { getSupabaseClient } from '../../../api/_lib.js';
...
export class SupabaseAdapter {
  ...
  async batchUpsertFinancialBasics(records) {
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      // 確保型別正確
      const fiscalYear = parseInt(record.fiscal_year);
      const taxId = record.tax_id ? String(record.tax_id).trim() : null;

      // 驗證必要欄位
      if (!fiscalYear || isNaN(fiscalYear) || !taxId) {
        results.skipped++;
        results.errors.push({
          row: i + 1,
      ...
      // 覆寫確保型別
      const cleanRecord = { ...record, fiscal_year: fiscalYear, tax_id: taxId };
