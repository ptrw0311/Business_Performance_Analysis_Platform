...
async function fetchData(tableName) {
  ...
  const result = await client.execute({
    sql: `SELECT * FROM ${tableName} ORDER BY fiscal_year DESC, tax_id ASC`,
  });
...
async function main() {
  ...
  try {
    // 匯出 financial_basics
    console.log('正在查詢 financial_basics...');
    const financialData = await fetchData('financial_basics');
    console.log(`  找到 ${financialData.length} 筆資料`);
    ...
    // 匯出 pl_income_basics
    console.log('\n正在查詢 pl_income_basics...');
    const plIncomeData = await fetchData('pl_income_basics');
    console.log(`  找到 ${plIncomeData.length} 筆資料`);
