// Vercel Serverless Function: 損益表單筆資料 CRUD API
// PUT: 更新指定 (taxId, year) 的損益表資料
// DELETE: 刪除指定 (taxId, year) 的損益表資料
...
export async function PUT(request, { params }) {
  try {
    ...
    if (error) {
      ...
      return errorResponse('更新損益表失敗: ' + error.message, 500);
    }

    if (!data || data.length === 0) {
      return errorResponse('找不到指定的損益表資料', 404);
    }
    ...
    console.error('PUT pl-income 錯誤:', error);
...
export async function DELETE(request, { params }) {
  try {
    ...
    if (error) {
      ...
      return errorResponse('刪除損益表失敗: ' + error.message, 500);
    }
    ...
    console.error('DELETE pl-income 錯誤:', error);
    return errorResponse('伺服器錯誤', 500);
  }
}
